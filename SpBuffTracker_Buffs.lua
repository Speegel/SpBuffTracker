...
function SpBT:GetBuffNameFromTexture(texture)
    if self.buffTextureMap[texture] then
        ...
        -- Return a shortened version of the texture path for unknown buffs
        local shortTexture = string.gsub(texture, "Interface\\Icons\\", "")
        return shortTexture
    end
...
-- Update all player buffs - using vanilla WoW 1.12 API with texture mapping
function SpBT:UpdateAllBuffs()
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cFF66CCFFSpBuffTracker: Checking player buffs using vanilla API|r")
    ...
    while buffId >= 0 do
        ...
        -- Get additional buff information
        local timeLeft = GetPlayerBuffTimeLeft(buffId)
        local applications = GetPlayerBuffApplications(buffId)
        
        -- Debug info
        DEFAULT_CHAT_FRAME:AddMessage(string.format("|cFFBBBBBBSpBuffTracker Debug: Found buff: %s (%.1f sec)|r", buffName, timeLeft))
    ...
    DEFAULT_CHAT_FRAME:AddMessage(string.format("|cFF66CCFFSpBuffTracker: Found %d active buffs to track|r", buffIndex - 1))
...
-- Layout buff frames - improved simple version
